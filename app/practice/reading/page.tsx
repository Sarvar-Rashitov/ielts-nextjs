...
export default function ReadingPractice() {
  ...
  const isUnlocked = (plan: string, setName: string, packName: string, range: [number, number]) => {
    const p = plan.toLowerCase()
    if (p === "free") return range[0] <= 2  // free: first 2 tests (Package 1, Set 1)
        ...
        <div key={i} className="mb-10">
          ...
          <div className="grid grid-cols-3 gap-3">
            {set.packs.map((pack, j) => {
              const unlocked = isUnlocked(subscription || "free", set.name, pack.name, pack.range)
